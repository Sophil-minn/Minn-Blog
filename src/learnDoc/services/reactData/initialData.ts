      ...
      },
      {
        ...
        detail: {
          ...
          theory: [
            ...
            {
              title: '发送 POST 请求',
              concept: '当你决定将某些逻辑放入事件处理函数还是 Effect 中时，你需要回答的主要问题是：从用户的角度来看它是 怎样的逻辑。如果这个逻辑是由某个特定的交互引起的，请将它保留在相应的事件处理函数中。如果是由用户在屏幕上 看到 组件时引起的，请将它保留在 Effect 中'
            },
            {
              title: '链式计算 ',
              concept: '尽可能在渲染期间进行计算，以及在事件处理函数中调整 state',
              tips: '在某些情况下，你 无法 在事件处理函数中直接计算出下一个 state。例如，试想一个具有多个下拉菜单的表单，如果下一个下拉菜单的选项取决于前一个下拉菜单选择的值。这时，Effect 链是合适的，因为你需要与网络进行同步。'
            },
            {
              title: "初始化应用 ",
              concept: '如果某些逻辑必须在 每次应用加载时执行一次，而不是在 每次组件挂载时执行一次，可以添加一个顶层变量来记录它是否已经执行过了'
            },
            {
              title: '通知父组件有关 state 变化的信息',
              concept: '“状态提升” 允许父组件通过切换自身的 state 来完全控制子组件。这意味着父组件会包含更多的逻辑，但整体上需要关心的状态变少了。每当你尝试保持两个不同的 state 变量之间的同步时，试试状态提升！'
            },
            {
              title: '将数据传递给父组件 ',
              concept: '让父组件获取那些数据，并将其 向下传递 给子组件,数据从父组件流向子组件'
            },
            {
              title: '订阅外部 store',
              concept: '有时候，你的组件可能需要订阅 React state 之外的一些数据。这些数据可能来自第三方库或内置浏览器 API。由于这些数据可能在 React 无法感知的情况下发变化，你需要在你的组件中手动订阅它们。这经常使用 Effect 来实现'
            },
            {
              title: '获取数据',
              concept: '当你不得不编写 Effect 时，请留意是否可以将某段功能提取到专门的内置 API 或一个更具声明性的自定义 Hook 中，比如上面的 useData。你会发现组件中的原始 useEffect 调用越少，维护应用将变得更加容易'
            }
          ],
          summary: [
          ...
          theory: [
            {
              ...
              concept: ' Effects 挂载,重新挂载和卸载,而不是更新, 在挂载和重新挂载 Effect 时,都会运行 Effect 中的逻辑并指定一个清理函数,在卸载组件时,将调用最终的清理函数来卸载 Effect,为确保 Effect 与最新的依赖项 props/state 同步,请确保指定所有依赖项。否则,Effect 可能不会在需要时重新运行,您可以在 Effect 中返回的清理函数内执行任何必要的“卸载”逻辑,例如取消订阅或清除定时器。'
            },
          ],
        }
      },
